"use strict";
function findDiffFileArticle(articleChildElement) {
    return articleChildElement.closest('[data-qa="pr-diff-file-styles"]');
}
function findDiffFileArticleNameDiv(diffFileArticleElement) {
    const result = diffFileArticleElement.previousElementSibling;
    return result instanceof HTMLElement ? result : undefined;
}
function findEditIcon(parentElement) {
    var _a, _b;
    return ((_b = (_a = parentElement
        .querySelector('[d="M4.01095129 11.4162698c-.03460638.1588194.01397015.3242009.1292215.4399418.11525135.1157409.28167627.1662741.44271745.1344265l1.85519694-.3803763-2.04744532-2.0492777-.37969057 1.8552857zm6.67101901-7.13015493C10.5011218 4.10314173 10.2533609 4 9.99468235 4c-.25867857 0-.50643946.10314173-.68728798.28611487L4.55405299 9.03654019l2.40791105 2.41015971 4.75334136-4.75090076c.3795928-.38217768.3795928-.99478477 0-1.37696244l-1.0333351-1.03272183z"]')) === null || _a === void 0 ? void 0 : _a.closest("span")) !== null && _b !== void 0 ? _b : undefined);
}
function findActionsContainer(fileActionsElement) {
    return fileActionsElement.parentElement;
}
function findCopyPathButton(parentElement) {
    var _a, _b;
    return ((_b = (_a = parentElement
        .querySelector('[d="M5 16V4.992C5 3.892 5.902 3 7.009 3H15v13H5zm2 0h8V5H7v11z"]')) === null || _a === void 0 ? void 0 : _a.closest("button")) !== null && _b !== void 0 ? _b : undefined);
}
function findButtonIcon(button) {
    return button.firstElementChild;
}
function getBranchNames() {
    const branches = document.querySelector('[data-qa="pr-branches-and-state-styles"]');
    if (!branches) {
        console.warn("Branches div not found");
        return undefined;
    }
    const spans = branches.querySelectorAll('span[aria-hidden="true"]');
    if (spans.length !== 2) {
...
function getRepoOrigin() {
    const url = window.location.href;
    if (!url.startsWith(BITBUCKET_HOST)) {
        console.warn(`The URL of the PR page is not started from ${BITBUCKET_HOST}`);
        return undefined;
    }
    const path = url.slice(BITBUCKET_HOST.length);
    const pathArray = path.split("/");
    if (pathArray.length < 2) {
        console.warn("The URL of the PR page doesn't have organization or repository");
        return undefined;
    }
...
function addButtons(fileActionsElement, repoOrigin, branchNames) {
    var _a, _b;
    ...
    if (!fileNameId.startsWith(CNG_PREFIX)) {
        console.warn("Id in name div for file action node not start from chg-packages");
        return;
    }
    ...
    if (copyPathButton) {
        link.setAttribute("class", (_a = copyPathButton.getAttribute("class")) !== null && _a !== void 0 ? _a : "");
    ...
    for (const mutation of mutationList) {
        for (const node of mutation.addedNodes) {
            if (node instanceof HTMLElement) {
                const fileHeaderActions = node.querySelectorAll('[data-qa="bk-file__actions"]');
